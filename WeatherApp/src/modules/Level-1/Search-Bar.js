...
class SearchBarConstructor {
  constructor(elementRefManager, uniqueIdentifier, action = "#", method = "post", addDataList = false) {
    try {
      ...
      if(elementRefManager instanceof ElementRefManager) {
        ...
        throw new ReferenceError(`Failed to create new '${this.constructor.name}' class instance, the supplied element reference manager is not an instance of the 'ElementRefManager' class, received '${elementRefManager}'`)
      }
      ...
      if(isUniqueIdentifierValid) {
        ...
        throw new ReferenceError(`Failed to create new '${this.constructor.name}' class instance, the supplied 'unique identifier' failed validation, must be a non empty string without any spaces within it, received '${uniqueIdentifier}' as the supplied unique identifier`)
      }

      if(isActionValid) {
        this.#configData.action = action;
      } else {
        throw new ReferenceError(`Failed to create new '${this.constructor.name}' class instance, the supplied 'action' failed validation, must be a non empty string without any spaces within it, received '${action}' as the supplied action`)
      }

      if(isMethodValid) {
        this.#configData.method = method;
      } else {
        throw new ReferenceError(`Failed to create new '${this.constructor.name}' class instance, the supplied 'method' failed validation, must be a non empty string without any spaces within it, received '${method}' as the supplied method`)
      }
      ...
      if(typeof addDataList === "boolean") {
        ...
        throw new TypeError(`Failed to create new '${this.constructor.name}' class instance, the supplied value for the 'addDataList' parameter is not a boolean, received '${addDataList}' with a data type of '${typeof addDataList}'`)
      }
  ...
  //elements will match what the config defines
    ...
    searchBarInput: () => {
      ...
      searchBarInput.setAttribute("name", `search-bar-input_${this.#configData}_var`) //This way the search bar instance can be both semantic and unique in nature

      if(this.#configData.addDataList) {
        searchBarInput.setAttribute("list", `search-bar-input-options-${this.#configData.uniqueIdentifier}`); //add an attribute based on whether a data list will be added to this search bar
      }
    ...
    dataList: () => {
        ...
        dataList.setAttribute("id", `search-bar-input-options-${this.#configData.uniqueIdentifier}`) //define the mandatory attribute that will link the corresponding data list to its input
  ...
  #constructSearchBar() {
    ...
    searchBarInputElementClassList = Array.from(searchBarInputElement.classList);
    ...
    this.#storeElementRefs(searchBarInputElementClassList[0], searchBarInputElement); //them as their unique identifiers within storage

    if(this.#configData.addDataList) { //condition to also add a data list if the configuration designates such
